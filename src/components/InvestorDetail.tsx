      ...
      <section className="card-surface rounded-2xl border border-zinc-800 p-5">
        ...
        <div className="mt-4 flex flex-wrap gap-6 border-t border-zinc-800 pt-4">
          <div>
            ...
            <div className="mt-0.5 font-mono text-lg text-zinc-100">{detail.settledCount}</div>
          </div>
          <div>
            <div className="text-xs text-zinc-500">Win rate</div>
            <div className="mt-0.5 font-mono text-lg text-zinc-100">{Math.round(detail.winRate * 100)}%</div>
          </div>
          <div>
            <div className="text-xs text-zinc-500">Net P&L</div>
            <div className={`mt-0.5 font-mono text-lg font-semibold ${pnlClass(detail.netPnl_raw)}`}>
              {signed(detail.netPnl_raw)}
            </div>
          </div>
        </div>
        <p className="mt-3 text-xs leading-relaxed text-zinc-600">
          Computed from this investor&apos;s real on-chain bets that the indexer returns (recent activity) —
          may not cover their entire history.
        </p>
      </section>
